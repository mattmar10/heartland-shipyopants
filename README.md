...
```bash
heartland-shipyopants$ sam build
```
...
```bash
heartland-shipyopants$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
heartland-shipyopants$ sam local start-api
heartland-shipyopants$ curl http://localhost:3000/
```
...
```bash
heartland-shipyopants$ sam logs -n HelloWorldFunction --stack-name heartland-shipyopants --tail
```
...
```bash
heartland-shipyopants$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name heartland-shipyopants
```
